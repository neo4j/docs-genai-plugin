...
The following provider configurations are for the xref:reference/functions-procedures.adoc#ai-text-embed[`ai.text.embed`] function and the xref:reference/functions-procedures.adoc#ai-text-embed-batch[`ai.text.embedBatch`] procedure.
...
The query `CALL ai.text.embed.providers()` shows you the list of the providers supported in the version of the plugin you are running.
...
This provider gives support for all models that uses the same request parameters and response fields as the https://docs.aws.amazon.com/bedrock/latest/userguide/titan-embedding-models.html[Titan text models].
