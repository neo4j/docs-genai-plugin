...
=== Functions and procedures to encode embeddings with AI providers
...
[TIP]
To store the embeddings created by this function and procedure as node or relationship properties on a Neo4j database, it is necessary to use the built-in procedures link:{neo4j-docs-base-uri}/operations-manual/current/procedures/#procedure_db_create_setnodevectorproperty[`db.create.setNodeVectorProperty`] and 
...
Once stored on properties, they can be queried using a link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/vector-indexes[vector index].
...
=== Procedures for interacting with external vector databases
